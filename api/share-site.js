...
export default async function handler(req, res) {
  ...
  try {
    const { clientName, docTitle, description, siteUrl, password, slug } = await readBody(req);
    ...
    if (slug && slug.trim()) {
      ...
      const existing = await list({ prefix: `documents/${docId}/` });
      if (existing.blobs.length > 0) {
        return res.status(409).json({ error: 'That link is already in use — pick a different one' });
      }
    } else {
